...
if (linkExclusao != null) { 
	for ( var i = 0; i < linkExclusao.length; i++ ) {
		(function(i){
			var id_pesquisa = linkExclusao[i].getAttribute('rel');

			if (linkExclusao[i].addEventListener){
		    	linkExclusao[i].addEventListener("click", function(){confirmaExclusao(id_pesquisa);});
			}else if (linkExclusao[i].attachEvent) { 
				linkExclusao[i].attachEvent("onclick", function(){confirmaExclusao(id_pesquisa);});
			}
		})(i);
	}
}
	...
	if (retorno){
	    ...
	    //Cria um formulário
			var formulario = document.createElement("form");
					formulario.action = "../../cadastro_completo/action_pesquisa.php";
